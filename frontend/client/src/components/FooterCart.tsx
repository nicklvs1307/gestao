...
const FooterCart: React.FC<FooterCartProps> = ({ items, total, onClick }) => {
  ...
  return (
    <div className="fixed bottom-[85px] md:bottom-6 left-4 right-4 md:left-1/2 md:-translate-x-1/2 md:max-w-lg z-[var(--z-sticky)] animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div 
        onClick={onClick}
        className="bg-slate-900 border border-white/10 rounded-xl p-2 flex justify-between items-center shadow-xl cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition-all duration-200"
      >
        ...
        <Button 
          ...
          className="h-11 px-5 rounded-lg text-xs uppercase italic tracking-widest flex items-center gap-2"
        >
          Ver Carrinho
