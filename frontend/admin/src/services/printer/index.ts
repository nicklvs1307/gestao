...
export const printCashierClosure = async (
  ...
  (sessionOrders || []).forEach((order: unknown) => {
    ...
    (o.items || []).forEach((item: unknown) => {
      const i = item as { quantity?: number; product?: { name?: string }; productName?: string; priceAtTime?: number; addonsJson?: unknown };
      totalItems += (i.quantity || 0);
      const productName = i.product?.name || i.productName || 'Produto';
