...
const WaiterPos: React.FC = () => {
    ...
    const handleProductClick = (product: Product) => {
        const hasOptions = (product.sizes && product.sizes.length > 0) || (product.addonGroups && product.addonGroups.length > 0);
                            ...
                            {selectedTable.items?.map(item => (
                                <div key={item.id} className="p-4 bg-slate-900 border-2 border-white/5 rounded-2xl flex justify-between items-center opacity-80">
                                    <div className="flex-1">
                                        <h4 className="font-black text-slate-300 text-xs uppercase italic leading-tight mb-1">{item.product?.name || item.productName || 'Produto'}</h4>
