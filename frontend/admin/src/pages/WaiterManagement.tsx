  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-3xl font-black text-foreground tracking-tighter uppercase italic leading-none">Minha Equipe</h1>
          <p className="text-muted-foreground text-xs font-bold uppercase tracking-widest mt-2 flex items-center gap-2">
            <Utensils size={14} className="text-primary" /> Gestão de Garçons e Atendimento de Salão
          </p>
        </div>
        <div className="flex gap-3">
            ...
            <Button onClick={handleAddClick} className="rounded-xl px-8 italic font-black h-12 shadow-xl shadow-primary/10">
      ...
      {isLoading && users.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {users.map(user => (
            <Card key={user.id} className="p-0 overflow-hidden border-2 border-border hover:border-primary/20 transition-all duration-300 hover:shadow-2xl bg-white group" noPadding>
                <div className="p-8 flex flex-col items-center text-center">
                    {/* Avatar Estilizado */}
                    <div className="relative mb-6">
                        <div className="w-20 h-20 rounded-[2rem] bg-slate-900 text-primary flex items-center justify-center shadow-xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 border-4 border-white">
                            <Utensils size={36} />
                        </div>
                    ...
                    <div className="w-full pt-6 border-t border-slate-50 flex gap-2">
                        <Button 
                            ...
                            className="flex-1 h-11 text-[10px] uppercase tracking-widest gap-2 rounded-xl italic font-black bg-muted"
                        >
