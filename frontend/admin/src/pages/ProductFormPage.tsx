        ...
        <div className="space-y-4 animate-in fade-in duration-500 pb-10 text-foreground">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 sticky top-0 bg-background/90 backdrop-blur-md z-40 py-2 border-b border-border">
                ...
                <div className="flex items-center gap-2 w-full lg:w-auto">
                    <Button variant="ghost" type="button" className="flex-1 lg:flex-none h-9 rounded-lg font-black uppercase text-[9px] text-muted-foreground" onClick={() => navigate('/products')}>CANCELAR</Button>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                ...
                <div className="xl:col-span-8 space-y-4">
                    ...
                    <div className="flex bg-muted p-1 rounded-xl gap-1 w-full overflow-x-auto no-scrollbar">
                        {[
                            ...
                            <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id as any)} className={cn("flex-1 min-w-[80px] py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5", activeTab === tab.id ? "bg-white text-foreground shadow-sm border border-border" : "text-muted-foreground hover:text-foreground/60")}>
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'geral' && (
                            <motion.div key="geral" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4">
                                ...
                                <Card className="p-4 border-border bg-white space-y-4 shadow-sm">
                                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                                        ...
                                        <div className="md:col-span-4">
                                            <Input label="Preço de Venda (R$)" type="number" step="0.01" {...register('price', { valueAsNumber: true })} className="h-10 text-xs font-black text-primary" icon={DollarSign} disabled={watch('sizes')?.length > 0} />
                                        </div>
                            ...
                            <motion.div key="tamanhos" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-2">
                                    {sizeFields.map((field, index) => (
                                        <Card key={field.id} className="p-3 border-border bg-white shadow-sm">
                                            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                                                <div className="md:col-span-5">
                                                    <select {...register(`sizes.${index}.globalSizeId`, { required: true })} onChange={(e) => { const selected = globalSizes.find(s => s.id === e.target.value); if (selected) setValue(`sizes.${index}.name`, selected.name); }} className="ui-input w-full h-9 text-[10px] font-black italic border border-border rounded-lg px-2"><option value="">Selecionar Tamanho...</option>{globalSizes.map(gs => <option key={gs.id} value={gs.id}>{gs.name}</option>)}</select>
                        ...
                        {activeTab === 'fiscal' && (
                            <motion.div key="fiscal" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-4">
                                <Card className="p-6 border-border bg-white space-y-6 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <Input label="Código SKU (Saipos)" {...register('saiposIntegrationCode')} className="h-11 font-black text-primary" placeholder="Ex: 11544566" />
                                    </div>
    ...
    return (
        <div ref={setNodeRef} style={style} className={cn("relative", isDragging && "z-50")}>
            <div className={cn(
                ...
                isInherited ? "bg-orange-50/50 border-orange-100" : "bg-white border-border hover:border-slate-300"
            )}>
                <div className="flex items-center gap-3">
                    {!isInherited && (
                ...
                <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" type="button" onClick={onView} className="h-8 w-8 bg-background text-muted-foreground hover:text-blue-600 rounded-lg"><Eye size={14} /></Button>
                    <Button variant="ghost" size="icon" type="button" onClick={onEdit} className="h-8 w-8 bg-background text-muted-foreground hover:text-primary rounded-lg"><Edit2 size={14} /></Button>
                    {!isInherited && (
                        ...
                        <div className="space-y-1 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                            {filteredAvailable.map(group => (
                                <div key={group.id} onClick={() => { onUpdate([...selectedGroups, group]); setSearchTerm(''); }} className="p-2.5 bg-white border border-orange-100 rounded-xl flex items-center justify-between cursor-pointer hover:border-orange-500 transition-all group/btn">
                                    <span className="text-[10px] font-black uppercase italic text-foreground truncate">{group.name}</span>
                                    <Plus size={12} className="text-orange-400 group-hover/btn:scale-125 transition-transform" />
                                </div>
                            ))}
                            {filteredAvailable.length === 0 && <p className="text-[8px] font-bold text-muted-foreground text-center py-4 uppercase">Nada encontrado</p>}
                        </div>
                    </Card>
                ) : (
                    <div className="space-y-4">
                        <Card className="p-4 border-border bg-white">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h4 className="text-[10px] font-black uppercase text-foreground italic">Dica de Gestão</h4>
                            </div>
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center bg-orange-50 p-4 rounded-2xl border border-orange-100">
                <div className="flex items-center gap-3">
                    <div className="bg-primary text-white p-2 rounded-xl shadow-lg shadow-orange-200">
