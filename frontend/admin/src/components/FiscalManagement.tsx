...
import {
    ...
    RotateCcw, Layers, ChevronDown, Eye, Zap
} from 'lucide-react';
...
type Tab = 'config' | 'invoices' | 'queue';
type FilterStatus = 'ALL' | 'AUTHORIZED' | 'REJECTED' | 'PENDING' | 'CANCELED';
...
const FiscalManagement: React.FC = () => {
    ...
    const [cancelData, setCancelData] = useState<CancelData>({ open: false, invoice: null, reason: '', loading: false });
    const [inutilizeData, setInutilizeData] = useState<InutilizeData>({ open: false, nNFInicio: '', nNFFim: '', reason: '', loading: false });
    const [cartaCorrecaoData, setCartaCorrecaoData] = useState<CartaCorrecaoData>({ open: false, invoice: null, corrections: '', loading: false });

    const [exportMonth, setExportMonth] = useState(new Date().getMonth() + 1);
    const [exportYear, setExportYear] = useState(new Date().getFullYear());
    const [exporting, setExporting] = useState(false);

    const [queueStatus, setQueueStatus] = useState<QueueStatus>({ pending: 0, processing: 0, failed: 0, completed: 0 });
    const [queueLoading, setQueueLoading] = useState(false);

    useScrollLock(cancelData.open || inutilizeData.open || cartaCorrecaoData.open || showReport);
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            if (activeTab === 'config') {
                ...
                const res = await api.get('/fiscal/invoices', { params: { status: 'PENDING' } });
                setInvoices(res.data);
            }
    ...
    const loadQueueStatus = async () => {
        ...
        try {
            const res = await api.get('/fiscal/queue/status');
    ...
    const validateCnpj = async (cnpj: string) => {
        ...
        try {
            ...
            if (res.data.valid && res.data.checked) toast.success('CNPJ válido na Receita Federal!');
        } catch (error) {
            setCnpjValid({ valid: false, error: 'Erro ao validar' });
        } finally { setCnpjValidating(false); }
    };
    ...
    const searchCep = async (cep: string) => {
        ...
        try {
            const res = await api.get(`/fiscal/cep/${cleanCep}`);
            if (res.data) {
                setConfig({
                    ...config,
        ...
        } catch (error) { toast.error('CEP não encontrado.'); }
    };
    ...
    const handleCancelInvoice = async () => {
        ...
        try {
            await api.post('/fiscal/invoices/cancel', {
    ...
    const handleInutilizeInvoice = async () => {
        if (!inutilizeData.nNFInicio || !inutilizeData.nNFFim || !inutilizeData.reason.trim()) {
            return toast.error('Números e motivo são obrigatórios.');
        }
        setInutilizeData(prev => ({ ...prev, loading: true }));
        try {
            await api.post('/fiscal/invoices/inutilize', {
                nNFInicio: parseInt(inutilizeData.nNFInicio),
                nNFFim: parseInt(inutilizeData.nNFFim),
                reason: inutilizeData.reason
            });
            toast.success('Numeração inutilizada com sucesso!');
            setInutilizeData({ open: false, nNFInicio: '', nNFFim: '', reason: '', loading: false });
        } catch (error: any) {
            toast.error(error.response?.data?.error || 'Erro ao inutilizar numeração.');
            setInutilizeData(prev => ({ ...prev, loading: false }));
        }
    };

    const handleCartaCorrecao = async () => {
        if (!cartaCorrecaoData.invoice || !cartaCorrecaoData.corrections.trim()) {
            return toast.error('Correções são obrigatórias.');
        }
        setCartaCorrecaoData(prev => ({ ...prev, loading: true }));
        try {
            await api.post('/fiscal/invoices/carta-correcao', {
                invoiceId: cartaCorrecaoData.invoice.id,
                corrections: cartaCorrecaoData.corrections
            });
            toast.success('Carta de correção enviada com sucesso!');
            setCartaCorrecaoData({ open: false, invoice: null, corrections: '', loading: false });
            loadData();
        } catch (error: any) {
            toast.error(error.response?.data?.error || 'Erro ao enviar carta de correção.');
            setCartaCorrecaoData(prev => ({ ...prev, loading: false }));
        }
    };

    const handleRetryManual = async (orderId: string) => {
        try {
            await api.post('/fiscal/queue/retry', { orderId });
            toast.success('Reenvio iniciado!');
            loadQueueStatus();
        } catch (error: any) {
            toast.error(error.response?.data?.error || 'Erro ao reenviar.');
        }
    };
    ...
    const loadMonthlyReport = async () => {
        setLoading(true);
        try {
            const res = await api.get('/fiscal/reports/monthly', { params: { month: exportMonth, year: exportYear } });
    ...
    const handleExportXmls = async () => {
        setExporting(true);
        try {
            const response = await api.get(`/fiscal/invoices/export/${exportMonth}/${exportYear}`, { responseType: 'blob' });
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                    {/* Certificado expiry alert */}
                    {certStatus?.daysUntilExpiry !== undefined && certStatus.daysUntilExpiry <= 30 && (
                        <div className={cn(
                            "flex items-center gap-2 px-3 py-2 rounded-xl text-[8px] font-black uppercase tracking-widest",
                            certStatus.daysUntilExpiry < 0
                                ? "bg-rose-500 text-white animate-pulse"
                                : "bg-amber-500 text-white"
                        )}>
                            {certStatus.daysUntilExpiry < 0 ? (
                                <><AlertTriangle size={12} /> Certificado EXPIRADO</>
                            ) : (
                                <><AlertCircle size={12} /> Certificado expira em {certStatus.daysUntilExpiry} dias</>
                            )}
                        </div>
                    )}
                    ...
                    <div className="flex bg-slate-100 p-1 rounded-xl gap-1">
                        {([
                            ...
                            { id: 'queue', label: 'Fila', icon: RotateCcw }
                        ] as const).map(tab => (
                            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn(
                                ...
                                {tab.id === 'queue' && queueStatus.failed > 0 && (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="relative">
                                        <Input
                                            label="CNPJ"
                                            value={config.cnpj || ''}
                                            onChange={e => { setConfig({...config, cnpj: e.target.value}); setCnpjValid(null); }}
                                        ...
                                        {cnpjValid?.valid && <CheckCircle className="absolute right-3 top-9 text-emerald-500" size={16} />}
                                        {cnpjValid?.valid === false && <AlertCircle className="absolute right-3 top-9 text-rose-500" size={16} />}
                                    </div>
                                ...
                                <div className="space-y-6 pt-6 border-t border-slate-50">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                        ...
                                        <div className="relative">
                                            ...
                                            />
                                            <button type="button" onClick={() => searchCep(config.zipCode)} className="absolute right-3 top-9 text-primary hover:text-primary/80" title="Buscar CEP">
                                                <Search size={16} />
                                            </button>
                                        </div>
                                    </div>
                                            ...
                                            <div className={cn("mt-2 px-2 py-1 rounded text-[8px] font-black uppercase",
                                                ...
                                                {certStatus.daysUntilExpiry < 0
                                                    ...
                                                    : certStatus.daysUntilExpiry <= 30
                                                        ...
                                                        : `Válido - expira ${certStatus.daysUntilExpiry} dias`}
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <Card className="p-8 border-slate-200 shadow-xl bg-white space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <Button fullWidth onClick={handleExportXmls} isLoading={exporting} variant="secondary" className="h-12 rounded-xl font-black uppercase text-[9px] tracking-widest italic">
                                    <Download size={14} className="mr-2" /> XMLs
                                </Button>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <Card className="p-4 bg-gradient-to-br from-amber-500 to-amber-600 text-white border-none">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-[7px] font-black text-amber-200 uppercase tracking-widest">Pendentes</span>
                            </div>
                                    ...
                                    {loading ? (
                                        ...
                                        filteredInvoices.map((inv: any) => (
                                            <tr key={inv.id} className="group hover:bg-slate-50/80 transition-all duration-300">
                                                ...
                                                <td className="px-4 py-3">
                                                    {inv.attemptLog && inv.attemptLog.length > 0 ? (
                                                        <span className="text-[8px] font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded">
                ...
                <>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <Card className="p-4 bg-gradient-to-br from-amber-500 to-amber-600 text-white border-none">
                            ...
                            <p className="text-2xl font-black italic tracking-tighter">{queueStatus.pending}</p>
                        </Card>
                        <Card className="p-4 bg-gradient-to-br from-blue-500 to-blue-600 text-white border-none">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-[7px] font-black text-blue-200 uppercase tracking-widest">Processando</span>
                            </div>
                            <p className="text-2xl font-black italic tracking-tighter">{queueStatus.processing}</p>
                        </Card>
                        <Card className="p-4 bg-gradient-to-br from-rose-500 to-rose-600 text-white border-none">
                            <div className="flex items-center justify-between mb-2">
                                <AlertCircle size={14} className="text-rose-200" />
                                <span className="text-[7px] font-black text-rose-200 uppercase tracking-widest">Falharam</span>
                            </div>
                            <p className="text-2xl font-black italic tracking-tighter">{queueStatus.failed}</p>
                        </Card>
                        <Card className="p-4 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white border-none">
                            <div className="flex items-center justify-between mb-2">
                                <CheckCircle size={14} className="text-emerald-200" />
                                <span className="text-[7px] font-black text-emerald-200 uppercase tracking-widest">Concluídas</span>
                            </div>
                            <p className="text-2xl font-black italic tracking-tighter">{queueStatus.completed}</p>
                        </Card>
                    </div>
                                    ...
                                    {loading ? (
                                        ...
                                        invoices.map((item: any) => (
                                            <tr key={item.id} className="group hover:bg-slate-50/80 transition-all">
                                                <td className="px-4 py-3 font-black text-sm text-slate-900 italic">{item.orderId?.slice(0, 8)}...</td>
                                                <td className="px-4 py-3">
                                                    <span className={cn(
                                                        "px-2 py-0.5 rounded text-[9px] font-black",
                                                        item.attempts >= 3 ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"
                                                    )}>
                                                        {item.attempts}/3
                                                    </span>
                                                </td>
                                                <td className="px-4 py-3 text-[9px] text-slate-500 max-w-[300px] truncate">{item.lastError || '-'}</td>
                                                <td className="px-4 py-3">
                                                    <span className={cn(
                                                ...
                                                <td className="px-4 py-3 text-[9px] text-slate-500">
                                                    {item.scheduledFor ? formatSP(item.scheduledFor, 'dd/MM HH:mm') : '-'}
                                                </td>
                                                <td className="px-4 py-3 text-right">
                                                    {item.status === 'PENDING' && item.attempts < 3 && (
                                                        <Button
                                                            onClick={() => handleRetryManual(item.orderId)}
                                                            variant="secondary"
                                                            className="px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest italic"
                                                        >
                                                            <Zap size={10} className="mr-1" /> Retry
                                                        </Button>
                                                    )}
                                                </td>
                                            </tr>
                            ...
                            <div className="p-6 space-y-4">
                                <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl">
                                    <p className="text-[9px] font-bold text-amber-700 uppercase tracking-widest">
                                        Esta ação envia evento de cancelamento (110110) à SEFAZ. Requer autorização do destinatário.
                                    </p>
                                </div>
                                <div className="space-y-2">
            ...
            <AnimatePresence>
                {inutilizeData.open && (
                    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-950/40 backdrop-blur-md" onClick={() => setInutilizeData({ open: false, nNFInicio: '', nNFFim: '', reason: '', loading: false })} />
                        <motion.div
                            initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
                            className="relative w-full max-w-md bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200"
                        >
                            <header className="px-6 py-5 bg-orange-50 border-b border-orange-100 flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 bg-orange-500 text-white rounded-xl flex items-center justify-center">
                                        <Layers size={20} />
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-black text-slate-900 italic uppercase tracking-tighter leading-none">Inutilizar Numeração</h3>
                            ...
                            <div className="p-6 space-y-4">
                                <div className="bg-orange-50 border border-orange-200 p-4 rounded-xl">
                                    <p className="text-[9px] font-bold text-orange-700 uppercase tracking-widest">
                                        Utilizado quando números foram gerados com erro e não foram autorizados.
                                    </p>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <Input label="Nº Inicial" type="number" value={inutilizeData.nNFInicio} onChange={e => setInutilizeData({...inutilizeData, nNFInicio: e.target.value})} placeholder="1" />
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        value={inutilizeData.reason}
                                        onChange={(e) => setInutilizeData({ ...inutilizeData, reason: e.target.value })}
                                        maxLength={255}
                                    />
                                </div>
                                <Button
                                    fullWidth
                                    onClick={handleInutilizeInvoice}
                                    disabled={inutilizeData.loading || !inutilizeData.reason.trim() || !inutilizeData.nNFInicio || !inutilizeData.nNFFim}
                                    isLoading={inutilizeData.loading}
                                    className="h-12 rounded-xl font-black uppercase text-[10px] tracking-widest bg-orange-500 hover:bg-orange-600"
                                >
                                    <Layers size={14} className="mr-2" /> Confirmar Inutilização
                                </Button>
                            </div>
                        </motion.div>
                    </div>
            ...
            <AnimatePresence>
                {cartaCorrecaoData.open && (
                    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-950/40 backdrop-blur-md" onClick={() => setCartaCorrecaoData({ open: false, invoice: null, corrections: '', loading: false })} />
                        <motion.div
                            initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
                            className="relative w-full max-w-md bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200"
                        >
                            <header className="px-6 py-5 bg-blue-50 border-b border-blue-100 flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 bg-blue-600 text-white rounded-xl flex items-center justify-center">
                                        <FileDigit size={20} />
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-black text-slate-900 italic uppercase tracking-tighter leading-none">Carta de Correção</h3>
                            ...
                            <div className="p-6 space-y-4">
                                <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl">
                                    <p className="text-[9px] font-bold text-blue-700 uppercase tracking-widest">
                                        Evento 110110 - Corrige dados sem alterar valores. Utilize para erros de digitação, dados complementares, etc.
                                    </p>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        value={cartaCorrecaoData.corrections}
                                ...
                                <Button
                                    ...
                                    disabled={cartaCorrecaoData.loading || !cartaCorrecaoData.corrections.trim()}
                                    isLoading={cartaCorrecaoData.loading}
                                    className="h-12 rounded-xl font-black uppercase text-[10px] tracking-widest bg-blue-600 hover:bg-blue-700"
                                >
                                    <FileDigit size={14} className="mr-2" /> Enviar Carta de Correção
                                </Button>
                            </div>
                        </motion.div>
                    </div>
                            ...
                            <div className="p-6 overflow-y-auto">
                                <div className="grid grid-cols-4 gap-3 mb-6">
                                    ...
                                    <Card className="p-4 bg-blue-50 border-blue-200">
                                        <p className="text-[8px] font-black text-blue-600 uppercase">Taxa Sucesso</p>
