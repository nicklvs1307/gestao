...
const NavigationLauncher: React.FC<NavigationLauncherProps> = ({ isOpen, onClose }) => {
    ...
    const categories: NavCategory[] = [
        ...
        },
        {
            ...
            icon: Users,
            items: [
                { label: "Cadastro de Clientes", path: "/customers", icon: UserCog },
                { label: "Cupons de Desconto", path: "/coupons", icon: Ticket },
            ]
        },
        {
            title: "Relatórios",
            ...
            items: [
                ...
                { label: "Desempenho por Atendente", path: "/reports/staff", icon: Users2 },
                { label: "Desempenho por Garçom", path: "/reports/waiters", icon: Users2 },
                { label: "DRE Gerencial", path: "/reports/dre", icon: Calculator },
                { label: "Faturamento por Dia", path: "/reports/billing", icon: BarChart3 },
                { label: "Itens Consumidos", path: "/reports/consumed-items", icon: ClipboardList },
                { label: "Itens Vendidos", path: "/reports/items", icon: ClipboardList },
                { label: "Tempo de Produção", path: "/reports/production", icon: Clock },
                { label: "Tempo por Status", path: "/reports/status-time", icon: Clock },
                { label: "Vendas por Área de Entrega", path: "/reports/delivery-areas", icon: MapPin },
                { label: "Vendas por Forma de Pagamento", path: "/reports/payments", icon: CreditCard },
                { label: "Vendas por Período", path: "/reports/period", icon: BarChart3 },
            ]
        },
        {
            title: "Estoque",
            icon: Warehouse,
            items: [
                { label: "Análise e Simulação de CMV", path: "/stock/cmv", icon: Calculator },
                { label: "Ficha Técnica", path: "/products", icon: FileText },
                { label: "Grupos de Ingrediente", path: "/ingredients/groups", icon: Layers },
                { label: "Histórico de Posição de Estoque", path: "/stock/history", icon: History },
                { label: "Ingredientes e Insumos", path: "/ingredients", icon: Package },
                { label: "Lista de Compras", path: "/stock/shopping-list", icon: ShoppingCart },
                { label: "Movimentações de Estoque", path: "/stock/moves", icon: MoveHorizontal },
                { label: "Notas de Entrada", path: "/stock/invoices", icon: Receipt },
                { label: "Ordem de Compra", path: "/stock/purchase-orders", icon: ClipboardList },
            ]
        },
        {
            title: "Dashboards",
            ...
            items: [
                { label: "Acompanhamento de Vendas", path: "/dashboard", icon: BarChart3 },
                { label: "Canais", path: "/dashboard/channels", icon: Share2 },
                { label: "Faturamento", path: "/dashboard/billing", icon: DollarSign },
                { label: "Vendas por Data / Hora", path: "/dashboard/hourly", icon: Clock },
            ]
        },
        {
            ...
            icon: Settings,
            items: [
                { label: "Áreas de Entrega", path: "/settings/delivery-zones", icon: MapPin },
                { label: "Canais de Venda e Integrações", path: "/integrations", icon: Share2 },
                { label: "Comandas", path: "/tables", icon: LayoutDashboard },
                { label: "Configurações", path: "/settings/general", icon: Settings },
                { label: "Dados da Loja", path: "/settings", icon: Database },
                { label: "Dados Fiscais", path: "/fiscal", icon: ShieldCheck },
                { label: "Entregadores", path: "/drivers", icon: Truck },
                { label: "Garçons", path: "/auth/waiters", icon: Users2 },
                { label: "Mesas", path: "/tables", icon: LayoutDashboard },
                { label: "Modelos de Impressão", path: "/settings/printing", icon: FileText },
                { label: "Motivos de Cancelamento", path: "/settings/cancellation-reasons", icon: X },
                { label: "Status da Venda", path: "/settings/sale-status", icon: Star },
                { label: "Turnos", path: "/settings/shifts", icon: Clock },
                { label: "Usuários e Permissões", path: "/users", icon: ShieldCheck },
            ]
        }
    ];
