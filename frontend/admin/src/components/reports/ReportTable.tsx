    ...
    <Card className="p-0 overflow-hidden border-slate-200 shadow-xl bg-white">
      <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
        ...
        {totalCount !== undefined && (
          <span className="text-[11px] font-medium text-slate-500">{totalCount} registros</span>
        )}
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[11px] font-semibold uppercase text-slate-500 tracking-wider border-b border-slate-100 bg-slate-50/30">
              {columns.map((col, i) => (
      ...
      {showingCount !== undefined && totalCount !== undefined && showingCount < totalCount && (
        <div className="p-3 text-center border-t border-slate-100">
          <span className="text-[11px] font-medium text-slate-500">Exibindo {showingCount} de {totalCount} registros</span>
        </div>
      )}
      {emptyState}
    </Card>
