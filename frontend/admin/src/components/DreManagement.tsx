...
const DreManagement: React.FC = () => {
    ...
    if (loading && !dreData) return (
        <div className="flex flex-col h-[60vh] items-center justify-center opacity-30 gap-4">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">Calculando Resultados Gerenciais...</span>
        </div>
    );
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 text-xs font-medium mt-1">Demonstrativo de Resultados do Exercício</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 w-full md:w-auto">
                    <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-100">
                        ...
                        <input type="date" className="bg-transparent border-none text-xs font-semibold outline-none text-slate-600" value={startDate} onChange={e => setStartDate(e.target.value)} />
                        <ChevronRight size={12} className="text-slate-500" />
                        <input type="date" className="bg-transparent border-none text-xs font-semibold outline-none text-slate-600" value={endDate} onChange={e => setEndDate(e.target.value)} />
                    </div>
                    <Button onClick={fetchData} size="sm" className="rounded-xl h-10 px-5 italic text-[10px]">FILTRAR</Button>
                    <Button variant="outline" size="icon" className="rounded-xl h-10 w-10 bg-white"><Download size={16} className="text-slate-500"/></Button>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <Card className="p-6 border-slate-100 bg-white hover:border-orange-500/20 transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 italic">Venda Bruta</span>
                            </div>
                            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-0.5">Total Recebido</p>
                        ...
                        <Card className="p-6 border-slate-100 bg-white hover:border-orange-500/20 transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="text-orange-600 flex flex-col items-end">
                                    <span className="text-[11px] font-semibold uppercase tracking-wider leading-none italic">CMV Total</span>
                                    <span className="text-[11px] font-medium">{dreData.cmvPercentage.toFixed(1)}% do FAT.</span>
                                </div>
                            </div>
                            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-0.5">Custo Insumos</p>
                        ...
                        <Card className="p-6 border-slate-100 bg-white hover:border-emerald-500/20 transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="text-emerald-600 flex flex-col items-end">
                                    <span className="text-[11px] font-semibold uppercase tracking-wider leading-none italic">Margem Bruta</span>
                                    <span className="text-[11px] font-medium">{(dreData.grossMargin * 100).toFixed(1)}% eficiência</span>
                                </div>
                            </div>
                            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 mb-0.5">Lucro Operacional</p>
                        ...
                        <Card className={cn("p-6 border-2 transition-all relative overflow-hidden group shadow-xl", dreData.netProfit >= 0 ? "bg-slate-900 border-emerald-500/30" : "bg-rose-600 border-rose-400")}>
                            <div className="flex justify-between items-start mb-4 relative z-10">
                                ...
                                <span className={cn("text-[11px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-lg border", dreData.netProfit >= 0 ? "text-emerald-400 border-emerald-500/20 bg-emerald-500/10" : "text-white border-white/20 bg-white/10")}>{(dreData.netMargin * 100).toFixed(1)}% Margem</span>
                            </div>
                            <p className={cn("text-[11px] font-semibold uppercase tracking-wider mb-0.5 relative z-10", dreData.netProfit >= 0 ? "text-slate-500" : "text-rose-100")}>Resultado Líquido</p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <Card className="lg:col-span-7 p-0 overflow-hidden border-slate-200 shadow-lg bg-white" noPadding>
                            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                                <h3 className="text-xs font-semibold text-slate-900 uppercase tracking-wider flex items-center gap-2"><div className="p-1.5 bg-rose-100 text-rose-600 rounded-lg"><ArrowDownCircle size={16} /></div> Despesas Operacionais</h3>
                            ...
                            <div className="p-6 space-y-6">
                                        ...
                                        <div key={category} className="group cursor-default">
                                            <div className="flex justify-between items-center mb-2">
                                                <div className="flex flex-col">
                                                    <span className="text-[11px] font-semibold text-slate-900 uppercase tracking-wider group-hover:text-orange-600 transition-colors">{category}</span>
                                                    <span className="text-[11px] font-medium text-slate-500 uppercase">Impacto: {impact.toFixed(1)}% do total</span>
                                                </div>
                                ...
                                {Object.keys(dreData.operatingExpenses.breakdown).length === 0 && (
                                    <div className="py-12 text-center opacity-20"><Info size={40} className="mx-auto mb-3" /><p className="text-[11px] font-semibold uppercase tracking-wider">Nenhuma despesa registrada</p></div>
                                )}
                            </div>
                        </Card>
                        ...
                        <div className="lg:col-span-5 space-y-8">
                            <Card className="p-8 border-emerald-100 bg-emerald-50/10 space-y-8">
                                <h3 className="text-sm font-bold uppercase text-emerald-900 flex items-center gap-3 border-b border-emerald-100/50 pb-4"><div className="p-2 bg-emerald-500 text-white rounded-lg shadow-lg shadow-emerald-100"><Target size={18}/></div> Auditoria de Eficiência</h3>
                                ...
                                <div className="space-y-6">
                                    <div className="bg-white p-6 rounded-3xl border border-emerald-100 shadow-sm relative overflow-hidden group">
                                        <div className="flex items-center gap-4 mb-4 relative z-10">
                                            ...
                                            <div>
                                                <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider leading-none mb-1">Status do CMV</p>
                                                <p className={cn("font-bold uppercase text-sm", dreData.cmvPercentage > 32 ? "text-rose-600" : "text-emerald-600")}>{dreData.cmvPercentage > 32 ? 'ACIMA DO RECOMENDADO' : 'DENTRO DA META'}</p>
                                            </div>
                                        </div>
                                        <p className="text-[11px] font-medium text-slate-500 leading-relaxed uppercase relative z-10">
                                    ...
                                    <div className="space-y-4">
                                        <h4 className="text-[11px] font-semibold text-slate-900 uppercase tracking-wider ml-1">Divisão do Faturamento</h4>
                                        <div className="space-y-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                                <div className="flex flex-col items-center p-2 bg-slate-50 rounded-xl"><span className="text-[11px] font-semibold uppercase text-orange-500">CMV</span><span className="text-[11px] font-semibold text-slate-900">{dreData.cmvPercentage.toFixed(1)}%</span></div>
                                                <div className="flex flex-col items-center p-2 bg-slate-50 rounded-xl"><span className="text-[11px] font-semibold uppercase text-rose-500">DESPESAS</span><span className="text-[11px] font-semibold text-slate-900">{((dreData.operatingExpenses.total / dreData.grossRevenue) * 100).toFixed(1)}%</span></div>
                                                <div className="flex flex-col items-center p-2 bg-slate-50 rounded-xl"><span className="text-[11px] font-semibold uppercase text-emerald-500">LUCRO</span><span className="text-[11px] font-semibold text-slate-900">{((dreData.netProfit / dreData.grossRevenue) * 100).toFixed(1)}%</span></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col gap-3">
                                <Button variant="outline" fullWidth className="h-14 rounded-2xl bg-white border-slate-200 uppercase tracking-widest text-[10px] font-black italic shadow-md group"><Printer size={18} className="mr-2 text-slate-500 group-hover:text-orange-500" /> Imprimir Demonstrativo</Button>
                                <Button variant="ghost" fullWidth className="h-14 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-orange-600 hover:bg-orange-50">Manual de Instruções Financeiras</Button>
                            </div>
                        </div>
                    </div>
                </>
