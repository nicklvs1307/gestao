...
const AVAILABLE_BLOCK_TYPES = [
  ...
  { type: 'items', label: 'Itens do Pedido', defaultOrder: 7 },
  { type: 'observations', label: 'Observações', defaultOrder: 8 },
  { type: 'totals', label: 'Totais', defaultOrder: 9 },
  { type: 'payment', label: 'Pagamento', defaultOrder: 10 },
  { type: 'change', label: 'Troco', defaultOrder: 11 },
  { type: 'footer', label: 'Rodapé', defaultOrder: 12 },
  { type: 'qrcode', label: 'QR Code', defaultOrder: 13 },
];

// Blocos visíveis por padrão na configuração inicial
...
class PrintLayoutService {
  ...
  /**
   * Cria a configuração padrão de layout para um restaurante
   * Usado na primeira acesso ou migração do localStorage
   */
  async createDefault(restaurantId, globalSettings = {}) {
    try {
        ...
        data: {
          ...
          blocks: {
            create: AVAILABLE_BLOCK_TYPES.map((block) => ({
              ...
              label: block.label,
              isVisible: DEFAULT_VISIBLE_BLOCKS.includes(block.type),
   ...
   */
  async createFromMigration(restaurantId, localData) {
    try {
   ...
   */
  async updateGlobalSettings(restaurantId, data) {
    try {
   ...
   */
  async updateBlocks(restaurantId, blocksData) {
    try {
   ...
   */
  async addCustomBlock(restaurantId, blockData) {
    try {
      const config = await prisma.printLayoutConfig.findUnique({
        where: { restaurantId },
        select: { id: true }
   ...
   */
  async removeBlock(restaurantId, blockId) {
    try {
      const config = await prisma.printLayoutConfig.findUnique({
        where: { restaurantId },
        select: { id: true }
  ...
  /**
   * Deleta toda a configuração de layout do restaurante
   */
  async deleteByRestaurant(restaurantId) {
    try {
