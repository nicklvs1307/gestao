...
const PRODUCTION_URL = 'https://api.focusnfe.com.br/v2';
const HOMOLOGATION_URL = 'https://homologacao.focusnfe.com.br/v2';

class FocusNFeProvider extends FiscalProvider {
    constructor(config) {
        super(config);
        this.baseURL = config.environment === 'production' ? PRODUCTION_URL : HOMOLOGATION_URL;
        this.authorization = Buffer.from(`${config.token}:`).toString('base64');
    }
    ...
    async send(xml, options = {}) {
        try {
            const url = `${this.baseURL}/nfe?ref=${this.config.orderId || Date.now()}`;
            ...
            return {
                success: false,
                error: response.data?.message || 'Erro desconhecido do FocusNFe',
                status: 'REJECTED',
                codes: { cStat: response.data?.status, xMotivo: response.data?.message }
            };
        } catch (error) {
            logger.error('[FocusNFe] Erro ao enviar:', error.message);
            return {
                success: false,
    ...
    async queryReceipt(receipt) {
        try {
            const url = `${this.baseURL}/nfe/${receipt}`;
            const response = await axios.get(url, { headers: this._headers(), timeout: 15000 });

            const data = response.data;
            if (data.status === '200' && data.nfe) {
                return {
                    success: true,
                    status: 'AUTHORIZED',
                    accessKey: data.nfe.chave_nfe,
            ...
            logger.error('[FocusNFe] Erro ao consultar:', error.message);
    ...
    async cancelEvent({ accessKey, protocol, reason, cnpj, timestamp }) {
        try {
            ...
            const url = `${this.baseURL}/nfe/${accessKey}/cancelamento`;
            ...
            logger.error('[FocusNFe] Erro cancelamento:', error.message);
    ...
    async correctionLetterEvent({ accessKey, protocol, sequence, corrections, cnpj, timestamp }) {
        try {
            ...
            const url = `${this.baseURL}/nfe/${accessKey}/carta-correcao`;
            const response = await axios.post(url, xml, { headers: this._headers(), timeout: 30000 });
            ...
            logger.error('[FocusNFe] Erro carta correção:', error.message);
            return { success: false, error: error.message };
        }
    }
    ...
    async inutilizationEvent({ nNFInicio, nNFFim, reason, cnpj, uf }) {
        try {
            ...
            const url = `${this.baseURL}/nfe/inutilizacao`;
            const response = await axios.post(url, xml, { headers: this._headers(), timeout: 30000 });
            ...
            logger.error('[FocusNFe] Erro inutilização:', error.message);
            return { success: false, error: error.message };
        }
    }
    ...
    async status() {
        try {
            const url = `${this.baseURL}/nfe/status`;
            const response = await axios.get(url, { headers: this._headers(), timeout: 10000 });
            return { online: response.status === 200, provider: 'focusNFe', environment: this.config.environment };
        } catch (error) {
            return { online: false, provider: 'focusNFe', environment: this.config.environment };
        }
    }
...
<eventoNFe xmlns="http://www.portalfiscal.inf.br/nfe">
    <infEvento>
        ...
        <detEvento>
            ...
            <xCondUso>A Carta de Correcao e disciplinada pelo Art. 58-B do Decreto 7.212/2010 e serve para: Corrigir erros发生在 na emissao do documento fiscal sem que para isso necessite de cancelamento da nota de origem.</xCondUso>
        </detEvento>
    </infEvento>
</eventoNFe>`;
    }
...
module.exports = FocusNFeProvider;
